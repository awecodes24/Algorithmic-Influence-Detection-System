...
def compute_features():
    ...
    df["posts_per_day"] = df.apply(
        lambda r: round(r["total_posts"] / max(r["account_age_days"], 1), 4), axis=1
    )

    df["follower_ratio"] = df.apply(
        lambda r: round(r["follower_count"] / max(r["following_count"], 1), 4), axis=1
    )

    df["followers_per_day"] = df.apply(
        lambda r: round(r["follower_count"] / max(r["account_age_days"], 1), 4), axis=1
    )
    ...
    df["favourites_ratio"] = df.apply(
        lambda r: (
            round(r["favourites_count"] / max(r["total_posts"], 1), 4)
    ...
    df["listed_ratio"] = df.apply(
        lambda r: round(r["listed_count"] / max(r["follower_count"], 1), 4), axis=1
    )

    df["log_favourites"] = np.log1p(df["favourites_count"])
    ...
    for _, row in df.iterrows():
        cursor.execute(
            ...
            INSERT OR REPLACE INTO features (
                ...
                favourites_ratio, listed_ratio , log_favourites
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?)
        """,
            (
                row["account_id"],
...
def get_feature_matrix():
    ...
    print(f"   {feature_matrix.shape[0]} accounts × {feature_matrix.shape[1]} features")
